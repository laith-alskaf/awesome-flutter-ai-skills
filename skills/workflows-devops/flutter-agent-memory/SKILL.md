...
When starting ANY new development session or resuming work, AI Agents MUST execute the following 8-step reading order before modifying any code:

```text
1. PROJECT_PROFILE.md  → Static Project Identity & Tech Stack (Flutter/Dart version, DB, Routing)
2. AGENTS.md           → Core Governance Laws, Quality Rules & Behavioral Constraints
3. CURRENT_STATE.md    → Active Work Goal, Context, Assumptions & Confidence Matrix
4. KNOWLEDGE_INDEX.md  → Fast Context Map to ADRs, Skills, and Source Folders
5. AGENTS_MEMORY.md    → Working Ledger, Milestones, Health Meter & Lessons Learned
6. Relevant ADRs       → Architectural Decision Records in decisions/
7. Relevant Skills     → Orthogonal Domain Skills in skills/
8. SESSION_LOG.md      → Chronological Session History
```
...
To prevent git history pollution and commit noise, `AGENTS_MEMORY.md` and `CURRENT_STATE.md` are updated **ONLY** under the following triggers:
...
Whenever an unexpected bug, memory leak, or architectural anti-pattern is resolved, the AI Agent MUST append a short entry to the `Lessons Learned & Anti-Regression Log` in `AGENTS_MEMORY.md` to prevent repeating past mistakes in future sessions.
