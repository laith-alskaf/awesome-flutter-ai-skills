...
#   .agent/AGENTS.md                 -> Framework governance rules
#   .agent/ROUTER_MANIFESTO.md       -> Agent routing & skill matrix
...
#   .agent/skills/                   -> All 49 skills (local copy)
#   .agent/templates/                -> All 21 code templates
#   .agent/checklists/               -> All 7 review checklists
#   .agent/decisions/                -> All 12 ADRs
#   .agent/anti-patterns/            -> All 5 anti-pattern catalogs
#   .agent/scripts/                  -> verify_architecture.dart
...
foreach ($file in @("AGENTS.md", "ROUTER_MANIFESTO.md")) {
    $src = Join-Path $frameworkRoot $file
    $dst = Join-Path $agentDir $file
    if (Test-Path $src) {
        Copy-Item -Path $src -Destination $dst -Force
        Write-Host "      [+] .agent/$file" -ForegroundColor Green
    }
}
...
$srcKI = Join-Path $frameworkRoot "templates\knowledge_index.md.template"
...
# ---------------------------------------------
Write-Host "[3/6] Copying framework resources directly into .agent/..." -ForegroundColor Yellow
...
foreach ($folder in $resourceFolders) {
    ...
    if (Test-Path $src) {
        ...
        Write-Host "      [+] .agent/$folder/ ($count files)" -ForegroundColor Green
    }
}

# ---------------------------------------------
# 6. COPY ALL 49 SKILLS INTO .agent/skills/
# ---------------------------------------------
Write-Host "[4/6] Copying all 49 skills directly into .agent/skills/..." -ForegroundColor Yellow
...
# 2. Read .agent/AGENTS.md (governance laws & quality standards)
# 3. Read .agent/CURRENT_STATE.md (confidence matrix -- if < 0.80, trigger flutter-grill-me)
# 4. Check .agent/KNOWLEDGE_INDEX.md (skill navigation map)
# 5. Check pubspec.yaml (active state management library detection)
#
...
#   [OK] .agent/AGENTS.md: Read | Architecture: Feature-First Clean Architecture
...
#
# SKILLS LOCATION: .agent/skills/ (49 skills available locally)
# TEMPLATES:       .agent/templates/
# CHECKLISTS:      .agent/checklists/
# ADRs:            .agent/decisions/
# VERIFY ARCH:     dart run .agent/scripts/verify_architecture.dart
"@
...
$checks = @(
    @{ Path = (Join-Path $agentDir "AGENTS.md");                   Label = ".agent/AGENTS.md" },
    @{ Path = (Join-Path $agentDir "ROUTER_MANIFESTO.md");         Label = ".agent/ROUTER_MANIFESTO.md" },
    ...
    @{ Path = (Join-Path $agentDir "templates");                   Label = ".agent/templates/" },
    @{ Path = (Join-Path $agentDir "decisions");                   Label = ".agent/decisions/" },
...
Write-Host ""
Write-Host "  [SKILLS]:   .agent/skills/ (49 skills available offline)" -ForegroundColor Cyan
Write-Host "  [VERIFY]:   dart run .agent/scripts/verify_architecture.dart" -ForegroundColor Cyan
